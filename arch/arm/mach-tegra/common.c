...
/* 
    debug_uart_switch=1 :  console uart through phone jack enable
    debug_uart_switch=0  :  phone jack audio out enable */

extern int uart_debug_switch; 
static int __init uart_debug_setup(char *p)
{
	uart_debug_switch = (int) memparse(p,&p);

	return 1;
}
__setup("uart_debug=", uart_debug_setup);
