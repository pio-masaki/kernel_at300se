					...
					struct kobj_attribute *attr, char *buf)
{
	...
	start += scnprintf(start, end - start, "%u\n", suspend_count);

	return start - buf;
}

static struct kobj_attribute suspend_count_attribute =
	__ATTR(count, 0444, suspend_count_show, NULL);
	...
	if (suspend_kobj) {
		if (sysfs_create_file(suspend_kobj, \
			...
			pr_err("%s: sysfs_create_file suspend type failed!\n",
								__func__);
		if (sysfs_create_file(suspend_kobj, \
						&suspend_count_attribute.attr))
			pr_err("%s: sysfs_create_file suspend count failed!\n",
								__func__);
	}
