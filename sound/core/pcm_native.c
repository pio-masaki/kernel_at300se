...
extern int pcm_startup_silence_update(void);
/*
 *
 */
...
snd_pcm_sframes_t snd_pcm_lib_write2(struct snd_pcm_substream *substream,
...
static void snd_pcm_post_prepare(struct snd_pcm_substream *substream, int state)
{
	...
	if( snd_pcm_playback_empty(substream) && (silence_ms = pcm_startup_silence_update()) ) {
		static int iteration = 0;
		char silence[192]={0};
		int ret = 0;

		if ( iteration++ % 10 == 0 ) {
			printk(KERN_DEBUG "%s: silence send...\n", __func__ );
		}

		for(i=0; i<silence_ms; i++) {
			ret = snd_pcm_lib_write2(substream, silence, bytes_to_frames(runtime,sizeof(silence)) );
			if( ret < 0 ) {
				printk(KERN_ERR "%s state=%d avail=%d\n", __func__ , runtime->status->state,
