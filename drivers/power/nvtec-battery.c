...
static int nvtec_battery_get_full_charge_capacity(nvtec_battery_dev *batt_dev, int16_t *value)
{
    return nvtec_read_word(batt_dev->nvtec_dev,
...
static int inline is_ac_online(void)
{
    ...
    if (g_batt_dev->ac_in_pin)
        return (~gpio_get_value(g_batt_dev->ac_in_pin)) & 0x1;
    else
        return 0;
}
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
        ret = nvtec_battery_get_full_charge_capacity(batt_dev, &value);
        if (ret != 0)
            return ret;
        val->intval = value;
        break;
                                        ...
                                        void *ignored)
{
    ...
    case NVTEC_EVENT_SYSTEM:
        low_batt_event = event_data >> 7;
		dev_info(batt_dev->nvtec_dev, "Low batt event = %d\n", low_batt_event);
		if(low_batt_event == 1 && !is_ac_online()) {
            wake_lock(&(batt_dev->wlock));
		}
