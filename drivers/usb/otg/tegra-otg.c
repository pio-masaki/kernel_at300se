...
static void irq_work(struct work_struct *work)
{
	...
	host_disabled = tegra->host_disabled;
	...
	if (!host_disabled && !(status & USB_ID_STATUS) && (status & USB_ID_INT_EN))
		to = OTG_STATE_A_HOST;
...
static ssize_t show_host_dis(struct device *dev, struct device_attribute *attr,
				 char *buf) 
{
	struct platform_device *pdev = to_platform_device(dev);
	struct tegra_otg_data *tegra = platform_get_drvdata(pdev);

	*buf = tegra->host_disabled ? '1' : '0';
	strcat(buf, "\n");
	return strlen(buf);
}

static ssize_t store_host_dis(struct device *dev, struct device_attribute *attr,
				 const char *buf, size_t count)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct tegra_otg_data *tegra = platform_get_drvdata(pdev);
	...
	unsigned long flags;
	unsigned long dis_host;
	unsigned long val;

	if (sscanf(buf, "%d", &dis_host) != 1 || dis_host < 0 || dis_host > 1)
		return -EINVAL;

	if (dis_host) {
		spin_lock_irqsave(&tegra->lock, flags);
		tegra->host_disabled = true;
		from = otg->state;
		spin_unlock_irqrestore(&tegra->lock, flags);
...
static DEVICE_ATTR(disable_host, 0644, show_host_dis, store_host_dis);
...
static int tegra_otg_probe(struct platform_device *pdev)
{
	...
	err = device_create_file(&pdev->dev, &dev_attr_disable_host);
	if (err) {
		dev_warn(&pdev->dev, "Can't register sysfs attribute\n");
		goto err_irq;
	}
