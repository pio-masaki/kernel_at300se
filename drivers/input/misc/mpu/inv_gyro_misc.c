...
#define DEF_SELFTEST_GYRO_FULL_SCALE         (0 << 3)//+-250dps
...
//	.gyro_sens   = 32768 / 250,
    .gyro_sens   = 32768 / (250L<<(DEF_SELFTEST_GYRO_FULL_SCALE>>3)),        
...
#else
	//fs = 2000UL;    /* assume +/- 2 mg as typical */
    fs = (2000UL << (DEF_SELFTEST_ACCL_FULL_SCALE >> 3));
#endif
	g_z_sign = 1;
	ret_val = 0;
...
#define ACCEL_THRESHOLD_MG  DEF_ACCEL_ST_SHIFT_MAX
{
    ...
    accel_bias[Z] = abs(abs(reg_avg[Z]*st->chip_info.multi) - (1L<<15)/(2L<<(DEF_SELFTEST_ACCL_FULL_SCALE>>3))*DEF_GYRO_PRECISION);
    thres = ACCEL_THRESHOLD_MG*(1L << 15)/(2L<<(DEF_SELFTEST_ACCL_FULL_SCALE>>3));
...
static int inv_check_6050_gyro_self_test(struct inv_gyro_state_s *st,
	...
	for (i = 0; i < 3; i++) {
        ...
        long thres;
        thres = GYRO_THRESHOLD_DPS*(1L << 15)*DEF_GYRO_PRECISION/(250L<<(DEF_SELFTEST_GYRO_FULL_SCALE>>3));
