...
#ifdef DISP_COLOR_TEMP
static int set_gamma = 1;
static int gamma_lut_red[256];
static int gamma_lut_green[256];
static int gamma_lut_blue[256];
static int gamma_def_red[256] = { 
	0,1,2,3,4,4,5,6,7,8,9,10,11,11,11,12,
	13,14,15,16,17,17,18,19,20,21,22,23,24,24,25,26,
	27,28,29,30,31,32,32,33,34,35,36,37,38,39,39,40,
	41,42,43,44,45,46,46,47,48,49,50,51,52,53,53,54,
	55,56,57,58,59,60,60,61,62,63,64,65,66,67,68,68,
	69,70,71,72,73,74,75,75,76,77,78,79,80,81,82,82,
	83,84,85,86,87,88,89,89,90,91,92,93,94,95,96,97,
	97,98,99,100,101,102,103,104,104,105,106,107,108,109,110,111,
	111,112,113,114,115,116,117,118,118,119,120,121,122,123,124,125,
...
/*
	125,126,127,128,129,130,131,132,133,133,134,135,136,137,138,139,
	140,140,141,142,143,144,145,146,147,147,148,149,150,151,152,153,
	154,154,155,156,157,158,159,160,161,162,162,163,164,165,166,167,
	168,169,169,170,171,172,173,174,175,176,176,177,178,179,180,181,
	182,183,183,184,185,186,187,188,189,190,190,191,192,193,194,195,
	196,197,198,198,199,200,201,202,203,204,205,205,206,207,208,209,
	210,211,212,212,213,214,215,217,218,219,220,220,221,222,223,224
*/
};

static int gamma_def_green[256] = 
{
...
/*
	...
	174,175,176,177,178,179,181,182,183,184,185,186,187,188,189,190,
...
static int gamma_def_blue[256] = 
{

	0,2,3,5,6,8,9,11,12,13,14,16,17,18,19,21,
	22,23,24,26,27,28,29,30,31,33,34,35,36,37,38,39,
	...
	154,155,156,157,157,158,159,160,161,162,163,164,165,166,167,168,
	169,170,171,172,172,173,174,175,176,177,178,179,180,181,182,183,
	184,185,185,186,187,188,189,190,191,192,193,194,195,196,196,197,
	...
	242,243,243,244,245,246,247,248,249,250,251,251,252,253,254,255
};
static int gamma_num = 256;
int start_cp = 0;
...
#ifdef DISP_COLOR_TEMP
void tegra_dc_set_color_palette(struct tegra_dc *dc, int force)
{
	int i,j;
	...
	lut = &dc->fb_lut;
	for (i = 0; i < 256; i++) {
		lut->r[i] = gamma_lut_red[i];
		lut->g[i] = gamma_lut_green[i];
		lut->b[i] = gamma_lut_blue[i];
	}
	...
	/* update color palette immediately , do when set_gamma_set() */
	if ( force ) {
		for(i = 0; i < 256; i++)
		{
		  int rgb;
...
int gamma_update(void)
{
	int i;
	
	if ( set_gamma == 0 ) {  /* DISABLE */
		for(i=0; i<256; i++ )
			gamma_lut_red[i] = gamma_lut_green[i] = gamma_lut_blue[i] = i;
	}
	else if ( set_gamma == 99 )
		; //use customized
	else  {
		for(i=0; i<256; i++ ) {
			gamma_lut_red[i] = gamma_def_red[i];
			gamma_lut_green[i] = gamma_def_green[i];
			gamma_lut_blue[i] = gamma_def_blue[i];
		}
	}
	...
	for(i=0;i<TEGRA_MAX_DC;i++) {
		if ( tegra_dcs[i] != NULL ) {
	        ...
	        if((tegra_dcs[i]->out->type == TEGRA_DC_OUT_RGB) && (tegra_dcs[i]->enabled == true)) {
			  tegra_dc_set_color_palette(tegra_dcs[i],1);
	        }
...
int set_gamma_set(const char *val, struct kernel_param *kp)
{
