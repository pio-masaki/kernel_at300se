...
static int emc_min_rate = 50 * 1000 * 1000; /* will cause min emc rate to 204MHz while idle, 
                                               will be lower than 204Mhz if enter early suspend */

module_param_named(emc_min_rate, emc_min_rate, int, S_IRUGO | S_IWUSR);
...
int tegra_dc_set_dynamic_emc(struct tegra_dc_win *windows[], int n)
{
	...
	if ( (new_rate < emc_min_rate ) ) {
		new_rate = emc_min_rate;
	}
