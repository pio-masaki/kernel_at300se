...
struct tps6591x_rtc {
	...
	bool			irq_en;
    bool   alarm_poweron_en;
    bool   alarm_wake_en;
    unsigned long alarm_poweron_secs;
    unsigned long alarm_wake_secs;
    struct mutex  lock;
...
static int tps6591x_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	if (tm->tm_sec == 59) {
		err = tps6591x_read_regs(dev, RTC_SECONDS_REG, sizeof(buff), buff);
		if (err < 0) {
			dev_err(dev, " %s :: failed to read time, %d\n", __FILE__, err);
			return err;
		}
...
static int tps6591x_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
    ...
    if (alrm->pending == 0x5A) {
        rtc->alarm_poweron_secs = seconds - rtc->epoch_start;
        rtc->alarm_poweron_en = true;
        dev_info(dev->parent, "Set poweron alarm to %lu seconds\n",
                 rtc->alarm_poweron_secs);
        return 0;
    }

    /* use pending=0xA5 to indicate suspend wake by alarm */
    if (alrm->pending == 0xA5) {
        rtc->alarm_wake_secs = seconds - rtc->epoch_start;
        rtc->alarm_wake_en = true;
        dev_info(dev->parent, "Set suspend wake alarm to %lu seconds\n",
                 rtc->alarm_wake_secs);
        return 0;
    }
...
static int __devinit tps6591x_rtc_probe(struct platform_device *pdev)
{
    ...
    wake_lock_init(&rtc->wake_lock, WAKE_LOCK_SUSPEND,
                   "alarm_wake_lock");
...
static int tps6591x_rtc_suspend(struct platform_device *pdev, pm_message_t state)
{
    struct tps6591x_rtc *rtc = dev_get_drvdata(&pdev->dev);
    struct rtc_time tm;
    ...
    if (rtc->alarm_wake_en && rtc->alarm_wake_secs) {
        tps6591x_rtc_read_time(&pdev->dev, &tm);
        rtc_tm_to_time(&tm, &seconds);

        seconds += rtc->alarm_wake_secs;
        rtc_time_to_tm(seconds, &alarm.time);
        alarm.enabled = 1;
        alarm.pending = 0;

        dev_info(&pdev->dev, "System will wake after %lu seconds\n",
                 rtc->alarm_wake_secs);
        tps6591x_rtc_set_alarm(&pdev->dev, &alarm);
    }
...
static int tps6591x_rtc_resume(struct platform_device *pdev)
{
    struct tps6591x_rtc *rtc = dev_get_drvdata(&pdev->dev);

    if (rtc->alarm_wake_en) {
        wake_lock_timeout(&rtc->wake_lock, 15 * HZ);
        rtc->alarm_wake_en = 0;
        rtc->alarm_wake_secs = 0;
    }
...
static void tps6591x_rtc_shutdown(struct platform_device *pdev)
{
    ...
    if (rtc->alarm_poweron_en && rtc->alarm_poweron_secs) {
        tps6591x_rtc_read_time(&pdev->dev, &tm);
        rtc_tm_to_time(&tm, &seconds);

        seconds += rtc->alarm_poweron_secs;
        rtc_time_to_tm(seconds, &alarm.time);
        alarm.enabled = 1;
        alarm.pending = 0;

        dev_info(&pdev->dev, "System will power on after %lu seconds\n",
                 rtc->alarm_poweron_secs);
        tps6591x_rtc_set_alarm(&pdev->dev, &alarm);
    } else {
        /* Clean RTC alarm interrupt before system shutdown*/
        tps6591x_rtc_alarm_irq_enable(&pdev->dev, 0);
    }
}
